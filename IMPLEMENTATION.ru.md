...
### Маршрутизация URL в 2.1

- Настраиваемые режимы адресации Rest, Method, ControllerMethod и Native.
- Одинаковые пути REST и gRPC; выбор обработчика по Content-Type и метаданным операции.
- Проверка соответствия параметров пути и Protobuf до создания контроллера; проверка контракта остаётся до десериализации.
- Переиспользуемые каналы и описания методов, без неограниченного кэша на каждый конкретный URL.
- Опциональное анонимное скачивание контракта независимо от авторизации бизнес-методов.
- Маршрутизация включена в детерминированный хэш. См. [миграцию и ограничения](docs/url-routing.md).

Локальная проверка 2.1: прошли 67 тестов без HTTP/3. Это не проверка развёртывания в кластере и не замер производительности.
